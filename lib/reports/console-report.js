...
var eventTypes = {
    ...
    testFinished: function (event) {
        ...
        if (event.asserts != null) {
            var msg = "[%d] [%s s] [%s] [%d left, %d total] %s: %d assert(s)";
            ...
            this.logger.logInfo(msg, [event.slaveNumber, duration, event.browserName || "default browser", event.browserRemainingTasks - 1, event.remainingTasks - 1, event.name, event.asserts]);
        } else {
