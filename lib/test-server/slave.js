...
var eventHandler = {
    "testStarted": "onTestUpdate",
    "testFinished": "onTestUpdate",
    "error": "onTestUpdate",
    "coverage": "onTestUpdate",
...
var Slave = function (spark, data, config) {
    this.config = config;
    this.spark = spark;
    ...
    spark.on('end', wrapInTryCatch(this, this.onSparkDisconnected));
    spark.on('data', wrapInTryCatch(this, this.onDataReceived));
};
...
Slave.prototype.onDataReceived = function (data) {
    var eventName = data.event;
    if (eventHandler.hasOwnProperty(eventName)) {
        var method = eventHandler[eventName];
        this[method](data);
    }
};
...
Slave.prototype.onPauseChanged = function (data) {
    var paused = !! data.paused; // makes sure it is a boolean
...
Slave.prototype.onSparkDisconnected = function () {
    if (this.spark) {
        this.spark = null;
...
Slave.prototype.isAvailable = function () {
    return this.spark && !(this.paused || this.currentTask);
};
