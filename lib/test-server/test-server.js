...
var TestServer = function (config, logger) {
    ...
    this.waitingSlaveControllers = {}; // map of slave controllers waiting for their corresponding slave
...
TestServer.prototype.addSlave = function (slave) {
    ...
    if (slave.controllerId) {
        var controller = this.waitingSlaveControllers[slave.controllerId];
...
TestServer.prototype.addSlaveController = function (controller) {
    var self = this;
    var id = controller.id;
    self.waitingSlaveControllers[id] = controller;
    controller.once('stop-waiting', function () {
        var value = self.waitingSlaveControllers[id];
        // only remove the expected controller, in case it was replaced
        if (value === controller) {
            delete self.waitingSlaveControllers[id];
            console.log("stop waiting for " + id);
        }
    });
};
