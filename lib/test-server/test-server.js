...
TestServer.prototype.listen = function (port, host, callback) {
    ...
    this.server.listen(port, host, function () {
        ...
        if (host != hostname && dns.lookupService) {
            // node 0.10 doesn't have dns.lookupService
            dns.lookupService(hostname, port, function (err, hostname, service) {
                console.log("dns.lookupService response: ", arguments);
                if (!err) {
                    self.hostname = hostname;
                }
                callback();
