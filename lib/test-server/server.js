...
var primusLogger = require('../logging/primus-logger.js');
...
var sparkConnection = function (spark) {
    var testServer = this;
    spark.once('data', function (data) {
        if (data.event == 'hello' && data.type == 'slave') {
            var newSlave = new Slave(spark, data, testServer.config);
 ...
 * Browsers connect to it through a primus channel.
...
var TestServer = function (config, logger) {
    ...
    this.primus.on('connection', sparkConnection.bind(this));
    this.primus.on('log', primusLogger('primus', this.logger));
