...
(function () {
    ...
    var socketStatusUpdater = function (status, info) {
        return function (param) {
            socketStatus = param ? status.replace('$', param.timeout) : status;
    ...
    primus.on('open', socketStatusUpdater('connected'));
    ...
    primus.on('close', socketStatusUpdater('disconnected'));

    if (config.onDisconnect) {
        primus.on('close', config.onDisconnect);
    }

    primus.on('reconnecting', socketStatusUpdater('reconnecting in $ ms...', 'disconnected'));
    ...
    // To send coverage, using a POST request rather than using primus is better for performance reasons
    ...
    attester.coverage = function (window) {
        var $$_l = window.$$_l;
        if ($$_l) {
            sendTestUpdate('coverage'); // notify the server through primus that we are sending coverage (so that it
            // will wait for it)
    ...
    // used for testing attester itself:
    attester.__disconnect = function () {
        primus.end();
    };
